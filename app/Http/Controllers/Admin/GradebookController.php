...
class GradebookController extends Controller
{
    public function index(Request $request): View|JsonResponse
    {
        ...
        $results = $this->applySorting(
            $this->applyFilters(Result::with('user'), $filters),
            $filters['sort']
        )->paginate(15);
    ...
    public function exportXlsx(Request $request): StreamedResponse
    {
        ...
        $rows = $this->applySorting(
            $this->applyFilters(Result::query()->with('user'), $filters),
            $filters['sort']
        )->get();

        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        $sheet->setTitle('Gradebook');
        ...
        $fileName = 'gradebook_toefl_' . now()->format('Ymd_His') . '.xlsx';

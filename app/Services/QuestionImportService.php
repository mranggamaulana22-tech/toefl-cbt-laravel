...
/**
 * Import massal soal UJIAN dari Excel (.xlsx), opsional dibundel bersama
 * audio dalam .zip. Mirip PracticeQuestionImportService, tapi dengan 2
 * perbedaan penting:
 *  1. Setiap soal terikat ke satu PaketSoal spesifik (paket_soal_id).
 *  2. Ada validasi KAPASITAS per kategori — karena tiap paket wajib pas
 *     50 listening/40 structure/50 reading, import tidak boleh membuat
 *     satupun kategori melebihi kapasitas paket tersebut.
 */
class QuestionImportService
{
    ...
    /**
     * Template sama persis strukturnya dengan Soal Latihan (140 baris
     * pre-filled sesuai config('exam.exam.sections'), 1 contoh per
     * kategori) — supaya admin tidak perlu belajar 2 format berbeda.
     * Template ini TIDAK spesifik ke satu paket; boleh dipakai untuk
     * paket manapun.
     */
    public function generateTemplate(): StreamedResponse
    {
        ...
        $sheet->setTitle('Template Soal Ujian');
    ...
    /**
     * Import soal ke dalam satu paket spesifik. Selain validasi per baris
     * (sama seperti practice), ada validasi tambahan: kapasitas kategori.
     * Kalau soal yang divalidasi + soal yang SUDAH ADA di paket ini untuk
     * kategori yang sama melebihi target (misal 50 untuk listening),
     * seluruh import ditolak — supaya paket tidak pernah kelebihan soal
     * dan tetap valid dipakai untuk sesi ujian.
     *
     * @return array{success: bool, created: int, audio_attached: int, audio_missing: array<int,string>, error_type: string, errors: array<int,string>}
     */
    public function import(PaketSoal $paketSoal, UploadedFile $file): array
    {
        ...
        try {
            ...
            if ($extension === 'zip') {
                ...
                if ($openResult !== true) {
                    Log::error("[{$referenceCode}] Gagal membuka file ZIP saat import soal ujian.", [
                        'paket_soal_id' => $paketSoal->id,
                        'file_name' => $originalName,
                        'zip_error_code' => $openResult,
                    ]);

                    return $this->failure('file_format', [
            ...
            try {
                ...
                Log::error("[{$referenceCode}] Gagal membaca file Excel saat import soal ujian.", [
                    'paket_soal_id' => $paketSoal->id,
                    'file_name' => $originalName,
                ...
                return $this->failure('file_format', [
                    'File Excel tidak bisa dibaca oleh sistem. Kemungkinan penyebabnya: file rusak, formatnya bukan .xlsx asli, atau file sedang dibuka di program lain saat diupload. Coba buka & simpan ulang filenya sebagai .xlsx, lalu upload kembali.',
                ]);
            }

            $sheet = $spreadsheet->getActiveSheet();
            $rows = $sheet->toArray(null, true, true, false);
            array_shift($rows); // baris header

            $errors = [];
            $parsedRows = [];
            $newCountByCategory = ['listening' => 0, 'structure' => 0, 'reading' => 0];
            ...
            foreach ($rows as $index => $row) {
                ...
                $parsedRows[] = $parsed;
                $newCountByCategory[$parsed['category']]++;
            }

            if (! empty($errors)) {
                Log::info("[{$referenceCode}] Import soal ujian ditolak karena validasi baris gagal.", [
                    'paket_soal_id' => $paketSoal->id,
                    'file_name' => $originalName,
            ...
            if (empty($parsedRows)) {
                return $this->failure('file_empty', [
            ...
            // Validasi kapasitas: soal existing di paket ini + soal baru
            // tidak boleh melebihi target per kategori (50/40/50 default).
            $sectionCounts = config('exam.exam.sections', ['listening' => 50, 'structure' => 40, 'reading' => 50]);
            $capacityErrors = [];

            foreach ($newCountByCategory as $category => $newCount) {
                if ($newCount === 0) {
                    continue;
                }

                $existingCount = Question::where('paket_soal_id', $paketSoal->id)
                    ->where('category', $category)
                    ->count();

                $target = (int) ($sectionCounts[$category] ?? 0);
                $totalAfterImport = $existingCount + $newCount;

                if ($totalAfterImport > $target) {
                    $availableSlot = max(0, $target - $existingCount);
                    $capacityErrors[] = "Kategori " . ucfirst($category) . ": paket \"{$paketSoal->nama}\" sudah punya {$existingCount} soal, file ini menambah {$newCount} soal lagi (total {$totalAfterImport}), padahal target maksimal {$target}. Sisa slot yang tersedia untuk kategori ini: {$availableSlot}.";
                }
            }

            if (! empty($capacityErrors)) {
                Log::info("[{$referenceCode}] Import soal ujian ditolak karena melebihi kapasitas paket.", [
                    'paket_soal_id' => $paketSoal->id,
                    'file_name' => $originalName,
                ]);

                return $this->failure('capacity', $capacityErrors);
            }
            ...
            DB::transaction(function () use ($parsedRows, $audioFiles, $paketSoal, &$created, &$audioAttached, &$audioMissing): void {
                foreach ($parsedRows as $row) {
                    $audioPath = null;

                    if ($row['category'] === 'listening' && $row['audio_filename']) {
                        if (isset($audioFiles[$row['audio_filename']])) {
                            $ext = pathinfo($row['audio_filename'], PATHINFO_EXTENSION) ?: 'mp3';
                            $storedName = 'questions/audio/' . uniqid('audio_') . '.' . $ext;
            ...
            Log::info("[{$referenceCode}] Import soal ujian berhasil.", [
                'paket_soal_id' => $paketSoal->id,
                'file_name' => $originalName,
                'created' => $created,
            ...
            Log::error("[{$referenceCode}] Import soal ujian gagal karena error tak terduga.", [
                'paket_soal_id' => $paketSoal->id,
                'file_name' => $originalName,
                'exception' => $e->getMessage(),
                'exception_class' => get_class($e),

{{--
    Partial: isi detail soal latihan — KHUSUS untuk modal.
...
<div class="flex flex-wrap items-center gap-x-5 gap-y-2 mb-5 pb-4 border-b border-slate-100 dark:border-white/10 text-sm">
    <div class="flex items-center gap-1.5">
        ...
        <span class="font-semibold text-slate-800 dark:text-slate-200">{{ ucfirst($practiceQuestion->category) }}</span>
    </div>
    <div class="flex items-center gap-1.5">
        <span class="text-slate-400 text-xs uppercase tracking-wide">Jawaban Benar</span>
        <span class="inline-flex items-center justify-center w-5 h-5 rounded bg-indigo-600 text-white text-xs font-bold">{{ $practiceQuestion->correct_answer }}</span>
    </div>
    <div class="flex items-center gap-1.5">
        <span class="text-slate-400 text-xs uppercase tracking-wide">Audio</span>
        <span class="font-semibold {{ $practiceQuestion->audio_path ? 'text-emerald-600' : 'text-slate-400' }}">
            {{ $practiceQuestion->audio_path ? 'Ada' : 'Tidak ada' }}
        </span>
    </div>
</div>
...
<div class="mb-5">
    ...
    <p class="whitespace-pre-line leading-relaxed text-slate-800 dark:text-slate-200 text-base">{{ $practiceQuestion->question_text }}</p>
</div>

@if($practiceQuestion->passage)
    <div class="mb-5 rounded-lg bg-slate-50 dark:bg-white/5 p-4">
        <p class="text-xs font-semibold uppercase tracking-wide text-slate-400 mb-2">Passage</p>
        <p class="whitespace-pre-line leading-relaxed text-slate-700 dark:text-slate-300 text-sm">{{ $practiceQuestion->passage }}</p>
    </div>
@endif
...
@if($practiceQuestion->audio_path)
    <div class="mb-5">
        ...
        @if($practiceQuestion->audio_transcript)
            <p class="whitespace-pre-line leading-relaxed text-slate-800 dark:text-slate-200 text-sm rounded-lg bg-slate-50 dark:bg-white/5 p-4">{{ $practiceQuestion->audio_transcript }}</p>
        @endif
    </div>
@endif
    ...
    <div class="space-y-1.5">
        @foreach(['a', 'b', 'c', 'd'] as $opt)
            <div class="flex items-center gap-3 rounded-lg px-3 py-2 text-sm
                ...
                <span class="flex-1">{{ $practiceQuestion->{'option_'.$opt} }}</span>

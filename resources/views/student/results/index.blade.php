...
.fixed.top-0.right-0 { animation: ambientFloat 9s ease-in-out infinite; }
.fixed.bottom-0.left-0 { animation: ambientFloat 12s ease-in-out infinite reverse; }
.results-card {
    animation: scaleIn 0.55s cubic-bezier(0.34, 1.56, 0.64, 1) both;
    transition: transform 0.25s cubic-bezier(0.34, 1.56, 0.64, 1),
                box-shadow 0.25s ease;
}
.results-card:hover { transform: translateY(-4px) scale(1.015); box-shadow: 0 16px 40px rgba(15, 23, 42, 0.10); }
...
.results-panel:hover { box-shadow: 0 0 32px 6px rgba(99, 102, 241, 0.10), 0 18px 50px rgba(15,23,42,0.08); }
.results-panel-head::after {
    content: ''; display: block; position: absolute; bottom: 0; left: 0; right: 0; height: 1px;
    background: linear-gradient(90deg, transparent 0%, rgba(99,102,241,0.35) 40%, rgba(99,102,241,0.35) 60%, transparent 100%);
    background-size: 200% 100%; animation: shimmer 3s linear infinite; opacity: 0.6;
}
.results-panel-head { position: relative; }
.results-table-row { transition: background-color 0.2s ease, transform 0.2s ease; }
.results-table-row:hover { transform: translateX(4px); }
.results-mobile-row { transition: transform 0.25s ease, box-shadow 0.25s ease; }
.results-mobile-row:hover { transform: translateY(-3px); box-shadow: 0 10px 30px rgba(15, 23, 42, 0.09); }
.results-mobile-mini { transition: transform 0.2s ease, box-shadow 0.2s ease; }
.results-mobile-mini:hover { transform: scale(1.04); box-shadow: 0 4px 14px rgba(15, 23, 42, 0.08); }
a.bg-indigo-600, a.bg-emerald-600 {
    transition: transform 0.2s cubic-bezier(0.34, 1.56, 0.64, 1), box-shadow 0.2s ease, background-color 0.2s ease;
}
a.bg-indigo-600:hover, a.bg-emerald-600:hover { transform: translateY(-2px) scale(1.03); box-shadow: 0 8px 20px rgba(99, 102, 241, 0.25); }
a.bg-indigo-600:active, a.bg-emerald-600:active { transform: scale(0.97); }
</style>

    <div class="results-page min-h-screen text-white font-sans selection:bg-purple-500/30 overflow-hidden py-8 pt-4 sm:pt-8 lg:pt-10" x-data="{ loaded: false }" x-init="$store.bg.enabled = true; loaded = true">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="results-panel overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_18px_50px_rgba(15,23,42,0.08)]">

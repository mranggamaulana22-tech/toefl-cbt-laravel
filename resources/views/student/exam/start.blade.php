                ...
                <div class="grid gap-6 p-6 md:p-8 lg:grid-cols-[1.4fr_0.9fr] lg:items-start">
                    ...
                    <div>
                        ...
                        <h3 class="anim-title mt-4 text-3xl font-black tracking-tight text-slate-900">
                            Instruksi ujian yang ringkas dan jelas.
                        </h3>
                        <p class="anim-desc mt-3 text-sm leading-7 text-slate-600">
                            Halaman ini membantu kamu memastikan sesi aktif, memahami aturan, dan masuk ke tes dengan keadaan siap.
                        </p>
                    ...
                    <div class="anim-card rounded-3xl border border-indigo-300 bg-gradient-to-br from-indigo-700 via-indigo-600 to-sky-600 p-6 text-white shadow-[0_16px_40px_rgba(30,64,175,0.30)]">
                        ...
                        <h4 class="mt-2 text-2xl font-black text-white">
                            {{ $setting->current_cycle > 0 ? 'Sesi #'.$setting->current_cycle : 'Sesi belum dimulai' }}
                        </h4>

                        <div class="mt-5 rounded-2xl px-4 py-3 {{ $canStart ? 'border border-white/20 bg-white/10' : 'border border-white/15 bg-white/5' }}">
                            @if ($canStart)
                                <p class="text-sm font-semibold text-violet-100">Sesi aktif dan siap dikerjakan.</p>
                            @elseif (!$setting->is_open)
                                <p class="text-sm font-semibold text-violet-100">Ujian belum dibuka oleh admin.</p>
                            @elseif ($hasAttemptedCurrentCycle)
                                <p class="text-sm font-semibold text-violet-100">Kamu sudah mengerjakan sesi aktif ini.</p>
                            @endif
                        </div>

                        <div class="mt-6 space-y-3 text-sm text-violet-100/90">
                            <p><span class="font-semibold text-white">Akses:</span> tes hanya dapat dibuka jika sesi aktif.</p>
                            <p><span class="font-semibold text-white">Submit:</span> jawaban akan dihitung secara otomatis saat dikirim.</p>
                        ...
                        <div class="mt-6 flex flex-col gap-3">
                            ...
                            @else
                                <button
                                    type="button"
                                    disabled
                                    class="inline-flex items-center justify-center rounded-2xl border border-white/15 bg-white/10 px-5 py-3 text-sm font-bold text-violet-100 cursor-not-allowed opacity-70"
                                >
                                    Belum Bisa Mulai Tes
                                </button>
                            @endif
                        </div>
                    </div>

...
<div class="flex items-center justify-between border-b border-white/[0.06] bg-[#111827] px-5 py-2.5">
    <div class="flex items-center gap-2.5">
        <div class="flex h-7 w-7 items-center justify-center rounded-lg bg-gradient-to-br from-indigo-500 to-violet-700 text-xs font-bold text-white shadow">T</div>
        <div>
            <div class="text-[13px] font-semibold text-white/85 leading-none">TOEFL Official Exam</div>
            <div class="mt-0.5 text-[10px] text-white/35 leading-none">Exam Mode &mdash; Hasil dinilai otomatis</div>
        </div>
    </div>
    <div class="flex items-center gap-4">
        <div class="flex items-center gap-2">
            <div class="flex h-7 w-7 items-center justify-center rounded-full border border-indigo-500/40 bg-indigo-500/15 text-[11px] font-semibold text-indigo-300">
                {{ strtoupper(substr(auth()->user()->name ?? 'U', 0, 2)) }}
            </div>
            <span class="text-[12px] text-white/50">{{ auth()->user()->name ?? 'Peserta' }}</span>
        </div>
        <div class="h-4 w-px bg-white/10"></div>
        <span class="inline-flex items-center rounded-md border border-red-500/30 bg-red-500/10 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-red-400">
            Ujian Resmi
        </span>
    </div>
</div>
...
<div class="grid grid-cols-3 items-center gap-4 border-b border-white/[0.06] bg-[#161f30] px-5 py-4">
    ...
    <div class="flex flex-col gap-2">
        ...
        <div class="flex flex-wrap items-center gap-1.5">
            <span class="inline-flex items-center rounded-md border border-indigo-500/30 bg-indigo-500/15 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-indigo-300"
    ...
    <div class="flex flex-col items-end gap-2">
        <div class="flex flex-col items-end gap-1">
            ...
            <div class="h-1.5 w-36 overflow-hidden rounded-full bg-white/[0.07]">
                <div class="h-full rounded-full bg-gradient-to-r from-indigo-500 to-violet-400 transition-all duration-300"

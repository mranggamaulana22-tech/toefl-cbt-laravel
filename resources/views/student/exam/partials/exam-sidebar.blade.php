    ...
    <div class="sticky top-6 rounded-3xl border border-white/10 bg-[#161f30] p-6 shadow-[0_18px_50px_rgba(0,0,0,0.4)]">
        ...
        <div class="grid grid-cols-5 gap-2">
            ...
            <button @click="goToQuestion({{ $index }})" :disabled="isAutoSubmitting"
                    :class="activeQuestion === {{ $index }} ? 'bg-indigo-600 text-white ring-4 ring-indigo-500/20' : (answers[{{ $index }}] ? 'bg-emerald-500 text-white' : 'bg-white/[0.06] text-white/40')"
        ...
        <div class="mt-8 space-y-2 text-xs">
            <div class="flex items-center gap-2 text-white/40"><span class="h-3 w-3 rounded-sm bg-indigo-600"></span> Posisi Sekarang</div>
